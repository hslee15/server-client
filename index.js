...
app.post('/posts',(req,res)=>{
    try {
        ...
        if(typeof subject!=='string' || subject.trim()==''||typeof desc !=='string'|| desc.trim()==''){
...
app.get('/posts/:id',(req,res)=>{
    try {
        ...
        return res.status(200).json({message:"게시글 전체 불러오기 완료",post: posts[index]})
    } catch (error) {
...
app.put('/posts/:id',(req,res)=>{
    try {
        ...
        return res.status(200).json({message:"게시글 1개 수정하기 완료",post: posts[index]})
    } catch (error) {
        console.error("게시글 1개 수정하는 중 오류",error)
        res.status(500).json({message:"서버 오류"})
    }
})
...
app.patch('/posts/:id/status',(req,res)=>{
    try {
        ...
        return res.status(200).json({message:"게시글 1개 상태변경 완료",post: posts[index]})
    } catch (error) {
